...
#model <- lm(exam_points ~ unique_practice_questions_answered , data = data)

# Create the faceted scatterplot and interaction plot
#interplot(m = model,var1= "Number of Practice Questions Answered" ,var2="Exam_Score") + 
 # xlab("Number of Practice Questions Answered") + 
 # ylab("Exam Score") + 
 # ggtitle("Interaction between Prior Listening and Practice on Exam Scores") +
 # facet_wrap(~ Prior_Listening)
#theme_few()
...
library(interplot)
 model_pract_pi_int <- lm(exam_points ~ unique_practice_questions_answered * proportion_of_prior_insutrction, data_z_scored)
interplot(m= model_pract_pi_int,var1="unique_practice_questions_answered",var2="proportion_of_prior_insutrction") + labs(title = "Unique Practice Questions vs Estimated Coefficient for Doing",
       x = "Unique Practice Questions Answered",
       y = "Estimated Coefficient for Doing") +
  theme_minimal()
```
